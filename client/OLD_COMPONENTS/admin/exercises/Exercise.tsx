...
export default function Exercise() {
  ...
  const onChange = (e: any) => {
    if (e.target.name === 'name')
      setExer((state: exercise) => {
        return { ...state, name: e.target.value.toUpperCase() }
      })
    setExer((state: exercise) => {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10vh', marginLeft: '3vw' }}>
      ...
      {exercise ? (
        <div style={{ display: 'flex', justifyContent: 'center', gap: '5vw', width: '100%' }}>
          ...
          >
            <img
              onClick={editExer ? onSave : onEdit}
              src={editExer ? save : edit}
              style={{ width: '15px', height: '15px', cursor: 'pointer', alignSelf: 'flex-end' }}
            />
            {editExer ? <ExerEdit exer={exercise} save={onChange} /> : <ExerDetail exer={exercise} />}
          </div>
        </div>
      ) : (
