  ...
  return (
    <>
      {detailEjec ? (
        <CardEjetcicio
          _id={detailEjec?._id}
          name={detailEjec?.name}
          difficulty={detailEjec?.difficulty}
          muscles={detailEjec?.muscles}
          genre={detailEjec?.genre}
          video={detailEjec?.video}
          description={detailEjec?.description}
        />
      ) : (
        <LoadingCards num={'1'} />
      )}
      <div>
            ...
            <div className='md:mt-10 mb-1  md:w-[90%] m-auto bg-gray-50  border border-gray-200'>
              ...
              <div className='flex md:flex-row flex-col-reverse items-center py-2 px-3 border-t dark:border-gray-600'>
                ...
                <div className='flex mx-5 md:my-0 my-4 items-center'>
                  <svg
                    onClick={() =>
                      rewind.cont >= 1 ? setrewind(pv => ({ ...pv, cont: 1 })) : setrewind(pv => ({ ...pv, cont: 1 }))
                    }
                    aria-hidden='true'
                    className={`w-6 h-6 cursor-pointer active:animate-ping ${
                      rewind.cont >= 1 ? 'text-yellow-400' : 'text-gray-500'
                    }`}
                    fill='currentColor'
                    viewBox='0 0 20 20'
                  ...
                  <svg
                    onClick={() =>
                      rewind.cont >= 2 ? setrewind(pv => ({ ...pv, cont: 1 })) : setrewind(pv => ({ ...pv, cont: 2 }))
                    }
                    aria-hidden='true'
                    className={`w-6 h-6 cursor-pointer active:animate-ping ${
                      rewind.cont >= 2 ? 'text-yellow-400' : 'text-gray-500'
                    }`}
                    fill='currentColor'
                    viewBox='0 0 20 20'
                    xmlns='http://www.w3.org/2000/svg'
                  ...
                  <svg
                    onClick={() =>
                      rewind.cont >= 3 ? setrewind(pv => ({ ...pv, cont: 2 })) : setrewind(pv => ({ ...pv, cont: 3 }))
                    }
                    aria-hidden='true'
                    className={`w-6 h-6 cursor-pointer active:animate-ping ${
                      rewind.cont >= 3 ? 'text-yellow-400' : 'text-gray-500'
                    }`}
                    fill='currentColor'
                    viewBox='0 0 20 20'
                    xmlns='http://www.w3.org/2000/svg'
                  ...
                  <svg
                    onClick={() =>
                      rewind.cont >= 4 ? setrewind(pv => ({ ...pv, cont: 3 })) : setrewind(pv => ({ ...pv, cont: 4 }))
                    }
                    aria-hidden='true'
                    className={`w-6 h-6 cursor-pointer active:animate-ping ${
                      rewind.cont >= 4 ? 'text-yellow-400' : 'text-gray-500'
                    }`}
                    fill='currentColor'
                    viewBox='0 0 20 20'
                    xmlns='http://www.w3.org/2000/svg'
                  ...
                  <svg
                    onClick={() =>
                      rewind.cont >= 5 ? setrewind(pv => ({ ...pv, cont: 4 })) : setrewind(pv => ({ ...pv, cont: 5 }))
                    }
                    aria-hidden='true'
                    className={`w-6 h-6 cursor-pointer active:animate-ping ${
                      rewind.cont >= 5 ? 'text-yellow-400' : 'text-gray-500'
                    }`}
                    fill='currentColor'
                    viewBox='0 0 20 20'
                    xmlns='http://www.w3.org/2000/svg'
                ...
                <div className='w-full flex justify-end items-end md:p-5'>
                  ...
                  {descripcionEjersicio?.find((e: any) => e?.email == user?.email) && (
                    <span
                      onClick={() => setValidac(false)}
                      className={` decoration-red-700 text-red-700 cursor-pointer`}
                    >
                      cancelar
                    </span>
                  )}
        ...
        <div className={`flex ${!validac && 'mt-20'}  flex-col md:py-5 bg-slate-200 w-[90%] m-auto`}>
          {detailEjec &&
            descripcionEjersicio?.map(({ _id, comment, rating, email, avatar }: any) => {
              return (
                <div key={_id} className='m-5 md:w-[40%] overflow-hidden bg-slate-100'>
                  <div className='flex'>
                    ...
                    <span className={`p-2 flex ${user?.email !== email && 'text-red-700'} text-lg justify-end w-full`}>
                      {user?.email !== email ? (
                        <span
                          onClick={() => SetBan(email)}
                          className="cursor-pointer  hover:after:content-['Banear'] after:p-2 after:absolute after:text-gray-500 duration-150"
                        >
                          <FaBan />
                        </span>
                      ) : (
                        <span
                          onClick={AddEdit}
                          className="cursor-pointer  hover:after:content-['Editar'] after:p-2 after:absolute after:text-gray-500 duration-150"
                        >
                          <AiFillEdit />
                        </span>
                      )}
