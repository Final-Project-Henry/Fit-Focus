...
const Login: React.FC<Propos> = ({ loading_icon, icon }) => {
  ...
  function handleSubmit(event: React.FormEvent): void {
    ...
    Activar ? dispatch(Activecuenta(Form_data)) : dispatch(User_Login_State(Form_data))
  }

  return (
    <div className='h-full w-full flex justify-center content-center'>
      <div>{typeof user.userToken === 'string' && user.userToken.length > 50 && <Navigate to='/fitFocus' />}</div>
      {/* component */}
      <div className='py-6 flex-1 content-center justify-center '>
        <div className='flex bg-white shadow-2xl overflow-hidden mx-auto sm:mt-0  max-w-sm h-auto lg:max-w-[68%]'>
          ...
          <div className='w-full p-8 lg:w-1/2'>
            ...
            <p className='text-xl text-gray-600 text-center'>{!Activar && 'Bienvenido de vuelta!'}</p>
            <div className='mt-4'>
              ...
              />
              {user.error?.includes('User') && <label className='text-red-500'>Esta cuanta no existe</label>}
            </div>
            <div className='mt-4'>
              ...
              <input
                ...
                onChange={event => handleChange(event)}
              />
              {user.error?.includes('Password') && <label className='text-red-500'>Contraseña incorrecta</label>}
            </div>
            ...
            <div className='mt-4 flex items-center justify-between'>
              ...
              <Link to='/auth/sing-up' className='text-blue-700 hover:underline dark:text-blue-500'>
                Crea una cuenta
              </Link>
              <span className='border-b w-1/5 md:w-1/4' />
            </div>
