...
export default function Training() {
  ...
  if (currentOrder == 29 && temp === 0) {
    setRun(false)
    Swal.fire('Terminado', 'Terminaste el entrenamiento completo, felicidades!!!!!!', 'success').then(() => {
      navigate('/fitFocus')
    })
  }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '5vw', justifyContent: 'center' }}>
            <label className='text-xl'>
              Tiempo estimado: <b>{users.rutines.reps === 'long' ? '25 min APROX.' : '21 min APROX.'}</b>
            </label>
            <label className='text-xl'>
              Dificultad:{' '}
              <b
                className={`${
                  users.rutines.difficulty === 'easy'
                    ? 'text-green-500'
                    : users.rutines.difficulty === 'medium'
                    ? 'text-yellow-500'
                    : 'text-red-500'
                }`}
              >
                {users.rutines.difficulty.toUpperCase()}
              </b>
            </label>
          </div>
                ...
                <div key={id++}>
                  <div style={{ display: 'flex', marginTop: '5vh' }}>
                    <div className='w-[25vw]'>
                      <img
                        className='rounded-l-2xl h-[50vh] w-[25vw]'
                        src={
                          !e.exer
                            ? e.exerc?.video
