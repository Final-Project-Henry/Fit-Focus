...
export default function VisualExerc(props: { datos: RutinObj }) {
  ...
  return (
    <div style={{ width: '30vw' }} className=' bg-zinc-100 shadow-lg overflow-auto'>
      ...
      >
        <h1 className='relative text-4xl font-semibold mt-5 shadow-sm px-3 rounded-md'>{props.datos.exerc?.name}</h1>
        ...
        <label className='mt-5'>
          <b className='font-semibold text-xl text-gray-600 border-b-[1px]'>•Musculos:</b>
          <span className='text-xl font-semibold ml-2 '>{props.datos.exerc?.muscles}</span>
        </label>
        ...
        <p className='text-xl font-medium w-[75%] font-sans text-gray-900 hover:text-gray-800 mt-1'>
          {props.datos.exerc?.description}
        </p>
      </div>
    </div>
  )
