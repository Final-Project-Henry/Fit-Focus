...
const HomeRegister = () => {
  ...
  return (
    <div className="max-w-screen-lg my-12 mx-auto overflow-hidden bg-slate-50 rounded-md">
      ...
      <br />
      <hr />
      <h1 className="text-center text-3xl mt-10 font-serif" id="excercises">
        Ejercicios del dia
      </h1>
      {exercises.length && (
        <div className="p-10 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-5">
          {exercises.map(({ video, name, difficulty, muscles, genre, _id }) => {
            return (
              <div
                key={_id}
                className={`rounded overflow-hidden shadow-lg cursor-pointer duration-300 hover:duration-200 hover:scale-105 hover:outline hover:outline-offset-1 ${
                  difficulty == "easy"
                    ? "outline-green-400"
                    : difficulty == "medium"
                    ? "outline-yellow-400"
