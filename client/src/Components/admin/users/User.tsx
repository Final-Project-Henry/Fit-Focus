...
export default function User() {
  ...
  useEffect(() => {
    ...
    if (users.admin_status === 'change') {
      Swal.fire('Completado', 'Cambio realizado con exito', 'success')
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '5vw', width: '100%' }}>
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'space-around' }}>
                  ...
                  <p>{user.name}</p>
                  <img
                    src={editProfile ? save : edit}
                    onClick={() => (editProfile ? onSaveChanges('profile') : onEdit('profile'))}
                    style={{ width: '15px', height: '15px', cursor: 'pointer', marginLeft: '15vw' }}
                  />
                </div>
                {editProfile ? (
              ...
              >
                ...
                <img
                  hidden={user?.userinfo.length === 0 ? true : false}
                  src={editInfo ? save : edit}
                  onClick={() => (editInfo ? onSaveChanges('info') : onEdit('info'))}
                  style={{ width: '15px', height: '15px', cursor: 'pointer', alignSelf: 'flex-end' }}
                />
