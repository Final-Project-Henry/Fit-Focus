...
import { Grid, GridColumn, GridToolbar, GridPageChangeEvent, GridSortChangeEvent } from '@progress/kendo-react-grid'
...
export default function Users() {
  ...
  useEffect(() => {
    if (users.delete_user === 'deleted') {
      Swal.fire('Eliminado', 'El Usuario fue eliminado de la DB exitosamente', 'success')
    ...
    <div style={{ marginLeft: '3vw' }}>
      {users.user_status === 'default' ? (
        ...
        <div>
          ...
          {grid}
          <GridPDFExport ref={pdfExport => (gridPDFExport = pdfExport)}>{grid}</GridPDFExport>
        </div>
      )}
    </div>
  )
