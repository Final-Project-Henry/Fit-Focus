...
const Carousel = ({ controls = true, velocity = '300', autoplay = false, interval = '5000', content = [] }: Props) => {
    ...
    <div className='w-[100%] mx-auto max-h-[400px] flex justify-center content-center overflow-hidden rounded-xl '>
      <div className='relative flex z-[1] content-center'>
        <div className='flex flex-nowrap max-h-full items-center content-center' ref={slideshow}>
          {content.map(({ src, text, stylesContent, stylesImage, stylesText }, i) => {
            return (
              <div
                key={i}
              ...
              >
                {text && (
                  <div className='absolute bg-[#111828c4] flex flex-col  w-full py-60 px-10 text-white z-10'>
                    <p className='font-extrabold  py-5 w-[50%] text-[2rem]'>{text}</p>
                    <Link
                      to='/mercadopago'
                ...
                {src.slice(-3) === 'mp4' ? (
                  ...
                  <img src={src} className={`w-full aling-top bg-cover ${stylesImage}`} alt='Img to slide' />
                )}
              </div>
            )
          })}
