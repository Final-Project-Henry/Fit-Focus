...
const Profile = () => {
  ...
  const handleClickUpload = async () => {
    ...
    try {
      const res = await fetch(`https://api.Cloudinary.com/v1_1/${CLOUD_NAME}/image/upload`, options)
          ...
          <ul className='flex flex-col pl-0 mb-0'>
            <li className='my-0.5 w-full'>
              <a
                ref={profile}
                id='profile'
                onClick={() => handleClickAside('profile')}
                className={`${
                  styles.selected === 'profile' && styles.a
                } rounded-lg duration-500 py-2.7 text-size-sm ease-nav-brand my-[3px] mx-4 flex items-center whitespace-nowrap px-4 transition-colors cursor-pointer sidebar`}
              >
                <div
                  onClick={() => handleClickAside('profile')}
                  className={`${
                    styles.selected === 'profile' && styles.div
                      ...
                      <g transform='translate(-1717.000000, -291.000000)' fill='#FFFFFF' fillRule='nonzero'>
                        <g transform='translate(1716.000000, 291.000000)'>
                          <g transform='translate(1.000000, 0.000000)'>
                            <path
                              onClick={() => handleClickAside('profile')}
                              className={`${styles.selected !== 'profile' && styles.path} duration-500 opacity-60`}
                              d='M45,0 L26,0 C25.447,0 25,0.447 25,1 L25,20 C25,20.379 25.214,20.725 25.553,20.895 C25.694,20.965 25.848,21 26,21 C26.212,21 26.424,20.933 26.6,20.8 L34.333,15 L45,15 C45.553,15 46,14.553 46,14 L46,1 C46,0.447 45.553,0 45,0 Z'
                            ></path>
                            <path
                              onClick={() => handleClickAside('profile')}
                              className={`${styles.selected !== 'profile' && styles.path} duration-500 `}
            ...
            <li className='my-0.5 w-full'>
              <a
                id='pay'
                onClick={() => handleClickAside('pay')}
                className={`${
                  styles.selected === 'pay' && styles.a
                } cursor-pointer rounded-lg py-2.7 text-size-sm ease-nav-brand my-[3px] mx-4 flex items-center whitespace-nowrap px-4 transition-colors duration-500 sidebar`}
              >
                <div
                  onClick={() => handleClickAside('pay')}
                  className={`${
                    styles.selected === 'pay' && styles.div
                  } shadow-soft-2xl mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center stroke-0 text-center xl:p-2.5`}
                >
                  <svg
                    width='12px'
                      ...
                      <g transform='translate(-2169.000000, -745.000000)' fill='#FFFFFF' fillRule='nonzero'>
                        <g transform='translate(1716.000000, 291.000000)'>
                          <g transform='translate(453.000000, 454.000000)'>
                            <path
                              onClick={() => handleClickAside('pay')}
                              className={`${styles.selected !== 'pay' && styles.path} opacity-60 duration-500`}
                              d='M43,10.7482083 L43,3.58333333 C43,1.60354167 41.3964583,0 39.4166667,0 L3.58333333,0 C1.60354167,0 0,1.60354167 0,3.58333333 L0,10.7482083 L43,10.7482083 Z'
                            ></path>
                            <path
                              onClick={() => handleClickAside('pay')}
                              className={`${styles.selected !== 'pay' && styles.path} duration-500`}
            ...
            <li className='cursor-pointer my-0.5 w-full'>
              <a
                ref={logOut}
                id='logOut'
                onClick={() => handleClickAside('logOut')}
                className={`${
                  styles.selected === 'logOut' && styles.a
                } rounded-lg duration-500 py-2.7 text-size-sm ease-nav-brand my-[3px] mx-4 flex items-center whitespace-nowrap px-4 transition-colors sidebar`}
              >
                <div
                  onClick={() => handleClickAside('logOut')}
                  className={`${
                    styles.selected === 'logOut' && styles.div
                  } duration-500 shadow-soft-2xl mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center stroke-0 text-center xl:p-2`}
                >
                  <svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24'>
                    <path
                      onClick={() => handleClickAside('logOut')}
                      className={`${styles.selected === 'logOut' && 'fill-white'} duration-500`}
            ...
            <li className='my-0.5 w-full'>
              <a
                ref={remove}
                id='remove'
                onClick={() => handleClickAside('remove')}
                className={`${
                  styles.selected === 'remove' && styles.a
                } cursor-pointer rounded-lg duration-500 py-2.7 text-size-sm ease-nav-brand my-[3px] mx-4 flex items-center whitespace-nowrap px-4 transition-colors sidebar`}
              >
                <div
                  onClick={() => handleClickAside('remove')}
                  className={`${
                    styles.selected === 'remove' && styles.div
                  } duration-500 shadow-soft-2xl mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center stroke-0 text-center xl:p-[7px]`}
                >
                  <svg
                    id='Layer_1'
                    version='1.1'
                  ...
                  >
                    ...
                    <g>
                      ...
                      >
                        <polygon
                          className={`${styles.selected === 'remove' && 'fill-white'} duration-500`}
                          id='Fill-6'
                        ...
                        />
                        <polygon
                          className={`${styles.selected === 'remove' && 'fill-white'} opacity-70 duration-500`}
                          id='Fill-9'
        ...
        <div className='w-full px-6 py-6 mx-auto loopple-min-height-78vh text-slate-500'>
            ...
            <div className='flex flex-wrap -mx-3'>
              <div className='flex-none w-auto max-w-full px-3'>
                <div className='text-size-base ease-soft-in-out h-18.5 w-18.5 relative inline-flex items-center justify-center rounded-xl text-white transition-all duration-200'>
                  <img
                    src={imageUrl}
                    className='rounded-3xl border-solid app_uploadButton w-[74px] h-[74px] shadow-soft-sm object-cover'
                  />
                  <div
                    onMouseEnter={() => handleHover(true, 'camera')}
                    onMouseLeave={() => handleHover(false, 'camera')}
                    className={`${
                      hover.hoverCamera && hover.div
                    } rounded-full absolute hover:duration-500 bottom-0 right-0 bg-white delay-100 duration-500 w-6 h-6 leading-8 overflow-hidden flex justify-center content-center`}
                  >
                    <input
                      type='file'
                      accept='image/*'
                      onChange={handlePreview}
                      className='app_uploadInput absolute scale-110 opacity-0'
                    />
                    <svg
                      height='19px'
                      width='19px'
                    ...
                    >
                      <g>
                        <path
                          className={`${hover.hoverCamera && hover.pathCamera} duration-500 delay-100 `}
                  ...
                  {hiddenButtons && (
                    <div className='w-[200px] flex justify-between'>
                      <div
                        ...
                        onMouseEnter={() => handleHover(true, 'upload')}
                        onMouseLeave={() => handleHover(false, 'upload')}
                        className={`bg-white cursor-pointer w-24 pl-2 rounded-2xl drop-shadow-[0_5px_3px_rgba(0,0,0,0.15)] `}
                      >
                        <svg
                          className={`${hover.hoverUpload && hover.pathUpload} delay-100 w-5 h-5 mb-[3px] inline`}
                          xmlns='http://www.w3.org/2000/svg'
                          xmlnsXlink='http://www.w3.org/1999/xlink'
                          version='1.1'
                          id='Capa_1'
                          x='0px'
                          y='0px'
                          viewBox='0 0 207.166 207.166'
                          xmlSpace='preserve'
                        >
                          <g>
                            <path
                              className={`duration-300 shadow-xl`}
                              d='M168.353,0H38.813c-4.971,0-9,4.029-9,9s4.029,9,9,9h129.54c4.971,0,9-4.029,9-9S173.324,0,168.353,0z'
                            />
                            <path
                              className={` duration-1000 shadow-xl`}
                              d='M109.947,44.028c-3.515-3.515-9.213-3.515-12.728,0L56.886,84.361c-3.515,3.515-3.515,9.213,0,12.728   c3.515,3.516,9.214,3.515,12.728,0l24.97-24.969v126.047c0,4.971,4.029,9,9,9s9-4.029,9-9V72.119l24.97,24.969   c1.757,1.758,4.061,2.636,6.364,2.636s4.606-0.879,6.364-2.636c3.515-3.515,3.515-9.213,0-12.728L109.947,44.028z'
                            />
                          </g>
                        </svg>
                        <button className='app_uploadButton'>Guardar</button>
                      </div>
                      <div
                        onClick={handleClickCancel}
                        onMouseEnter={() => handleHover(true, 'cancel')}
                        onMouseLeave={() => handleHover(false, 'cancel')}
                        className={`bg-white cursor-pointer w-24 pl-2 rounded-2xl drop-shadow-[0_5px_3px_rgba(0,0,0,0.15)] `}
                      >
                        <svg
                          className={`${
                            hover.hoverCancel && hover.pathCancel
                          } delay-100 w-4 h-4 mr-0.5 mb-[2px] inline`}
                          xmlns='http://www.w3.org/2000/svg'
                          xmlnsXlink='http://www.w3.org/1999/xlink'
                          version='1.1'
                          id='Layer_1'
          ...
          {styles.selected === 'profile' ? (
            <ProfileDetails
              ...
              age={state.user?.userinfo[0]?.age}
              equipment={state.user?.userinfo[0]?.equipment}
              experience={state.user?.userinfo[0]?.experience}
              genre={state.user?.userinfo[0]?.genre}
              goal={state.user?.userinfo[0]?.goal}
              height={state.user?.userinfo[0]?.height}
              weight={state.user?.userinfo[0]?.weight}
            />
          ) : /* : styles.selected === "progress" ? <Progress /> */
          styles.selected === 'remove' ? (
            <Remove />
          ) : styles.selected === 'pay' ? (
            <Pay plan={state.user?.plan} />
          ) : (
            styles.selected === 'logOut' && (
              <ProfileDetails
                name={state.user?.name}
                email={state.user?.email}
                plan={state.user?.plan}
            ...
            <div className='flex flex-wrap items-center -mx-3 lg:justify-between'>
              <div className='w-full max-w-full px-3 mt-0 mb-6 shrink-0 lg:mb-0 lg:w-1/2 lg:flex-none'>
                <div className='leading-normal text-center text-size-sm text-slate-500 lg:text-left'>
                  © 2022, Desarrollado por el equipo de<i className='fa fa-heart' aria-hidden='true'></i>
                  <a href='/contactanos' className='font-semibold text-slate-700'>
                    {' '}
                    FIT FOCUS{' '}
                  </a>
                  en Henry
              ...
              <div className='w-full max-w-full px-3 mt-0 shrink-0 lg:w-1/2 lg:flex-none'>
                <ul className='flex flex-wrap justify-center pl-0 mb-0 list-none lg:justify-end'>
                  <li className='nav-item'>
                    <a
                      href='/AboutUs'
                      className='block px-4 pt-0 pb-1 font-normal transition-colors ease-soft-in-out text-size-sm text-slate-500'
                    >
                      Acerca de nosotros
                    </a>
                  </li>
