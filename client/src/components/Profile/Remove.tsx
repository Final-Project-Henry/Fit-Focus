  ...
  function handleRemoveAccount(): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire('Desactivado', 'Tu cuenta ha sido desactivada exitosamente', 'success')
      }
    })
  }
            ...
            <div className='flex-auto p-4'>
              ...
              <ul className='flex flex-col pl-0 mb-0 rounded-lg'>
                <li className='relative block px-4 py-2 pt-0 pl-0 leading-normal bg-white border-0 rounded-t-lg text-size-sm text-inherit'>
                  <strong className='text-slate-700'>Esta accion es reversible</strong> &nbsp;{' '}
                </li>
                <li className='relative block px-4 py-2 pl-0 leading-normal bg-white border-0 border-t-0 text-size-sm text-inherit'>
                  {' '}
                  Si desactivas tu cuenta podras volver cuando desees, cuando vuelvas tu informacion y fotos estaran
                  igual que previo a desactivacion{' '}
                </li>
                <li className='relative flex justify-center px-4 py-2 pl-0 leading-normal bg-white border-0 border-t-0 text-size-sm text-inherit'>
                  <strong className='text-slate-700'></strong> &nbsp;
                  <button
                    onClick={handleRemoveAccount}
                    type='button'
                    className='delay-100 duration-300 inline-block px-6 py-2.5 bg-gray-700 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-red-600 hover:shadow-lg focus:bg-red-900 focus:shadow-lg focus:outline-none focus:ring-0 active:shadow-lg  ease-in-out'
                  >
                    Desactivar cuenta
                  </button>
                </li>
