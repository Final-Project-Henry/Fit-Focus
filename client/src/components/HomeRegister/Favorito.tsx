...
const Favoritos = () => {
  ...
  useEffect(() => {
    if (exercises?.length === 0) dispatch(Exercises_Get())
  }, [exercises])
  ...
  return (
    <>
      ...
      <div className='md:grid md:grid-cols-4 sm:grid-cols-1 m-8'>
        {user?.fav.length > 0 ? (
          dataFav?.length > 0 ? (
            dataFav?.map(({ _id, video, name, difficulty, muscles, genre, premium }) => {
              return (
                <div key={uuidv4()}>
                  <Link
                    key={_id}
          ...
          <div className='flex w-[140%] ml-[125%] justify-center bg-gray-300 rounded-sm p-2'>
            <p className='font-extrabold text-5xl text-gray-900'>No tienes favoritos todavía</p>
          </div>
        )}
      </div>
      <Footer2 />
