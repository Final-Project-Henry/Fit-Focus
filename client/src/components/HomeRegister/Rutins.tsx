    ...
    <div className={styles.container}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5vh' }}>
        ...
        <div style={{ display: 'flex', gap: '10vw' }}>
          <label>
            Tiempo estimado: <b>{props.reps === 'long' ? '25 min APROX.' : '21 min APROX.'}</b>
          </label>
          <label>
            Dificultad: <b>{props.dif?.toUpperCase()}</b>
          </label>
        </div>
        {props.rutins?.map((e: rutin) => {
          if (e.order) {
            return <Exerc key={e.exerc._id} exer={e?.exerc} time={e.time} order={e.order} />
          } else {
            return <Rest key={++id} time={e.time} />
          }
        })}
      </div>
    </div>
