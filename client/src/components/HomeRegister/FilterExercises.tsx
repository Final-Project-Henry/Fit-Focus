  ...
  return (
    <>
      <div className='bg-white flex  shadow flex-col sm:flex-col md:flex-row justify-around items-center w-[100%] sm:w-[100%] md:w-[80%] m-auto'>
        ...
        <div>
          <button
            className=' text-red-500 p-2 cursor-pointer m-5 py-0   text-sm text-back font-normal leading-loose  border-none outline-none  shadow-md active:shadow'
            onClick={() => handleSelectLimpiar()}
          >
            Limpiar filtro
          </button>
        </div>
      ...
      <div>
        <section className='md:grid md:grid-cols-4 sm:grid-cols-1'>
          {filtrado.length > 0 || exercises.length > 0 ? (
            filtrado.map(({ _id, video, name, difficulty, muscles, genre, premium }) => {
              return (
                <div key={uuidv4()} className=''>
                  <Link
                    key={_id}
                    to={premium && user?.plan == 'normal' ? `/mercadopago` : `/ejercicio/${_id}`}
                    className={`w-[98%] md:max-w-[74%] sm:w-[98%] min-h-[400px] sm:min-h-[400px] md:min-h-[240px] m-1 mt-5 sm:mt-5 sm:m-1  md:m-10  flex flex-col bg-white  shadow-md duration-150 cursor-pointer  hover:outline hover:outline-offset-1 ${
                      premium
                        ? 'outline-blue-400'
                        : difficulty == 'easy'
                        ? 'outline-green-400'
                  ...
                  >
                    {premium && user?.plan == 'normal' && (
                      <div className='flex flex-col justify-center items-center'>
                        <div className='absolute flex min-h-[10px] w-[10%] justify-center items-center z-10 '>
                          <br />
                          <img className='absolute z-10  w-[50%]' src={notPremiunImg} alt='' />
                          <Link
                            to='/mercadopago'
                    ...
                    >
                      <img
                        className='object-cover h-[400px] sm:h-[400px] md:h-[200px]  w-full'
                        src={premium && user?.plan == 'normal' ? notPremiunImg2 : video}
                        alt=''
                      />
                    </div>
                    <div className={`${premium && user?.plan == 'normal' ? 'blur-[5px]' : 'blur-0'}`}>
                      <h5 className='p-2 text-center text-xl font-bold -tracking-widest text-gray-900'>{name}</h5>
                    </div>
                    <div
                    ...
                    <div className={` text-center ${premium && user?.plan == 'normal' ? 'blur-[5px]' : 'blur-0'}`}>
                      <span
      ...
      {filtrado.length > 4 && filtrado.length < exercises.length ? (
        <div
          onClick={sumaDeAOcho}
          className='w-fill cursor-pointer border flex bg-[#111828e8] text-white hover:bg-black justify-center duration-200'
        >
          <button className='p-4'>Ver Mas ▽</button>
        </div>
      ) : (
        (exercises.length > 0 || filtrado.length > exercises.length) && (
          <div className='flex justify-center item-center text-5xl'></div>
        )
      )}
    </>
  )
