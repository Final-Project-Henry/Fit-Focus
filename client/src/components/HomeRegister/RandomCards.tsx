...
const RandomCards = ({ _id, video, name, difficulty, muscles, rating, genre, premium }: Props) => {
  ...
  return (
    <div>
      ...
      >
        ...
        <div className={`h-[300px] overflow-hidden  ${premium && user?.plan == 'normal' ? 'blur-[5px]' : 'blur-0'}`}>
          <img
        ...
        <div className={` text-center ${premium && user?.plan == 'normal' ? 'blur-[5px]' : 'blur-0'}`}>
          <span
            className={`inline-block ${
              difficulty == 'easy' ? 'bg-green-200' : difficulty == 'medium' ? 'bg-yellow-200' : 'bg-red-200'
